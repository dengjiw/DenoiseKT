...
class QueBaseModel(nn.Module):
    ...
    def batch_to_device(self,data,process=True):
        ...
        data_new = {}
        data_new['cq'] = torch.cat((dcur["qseqs"][:,0:1], dcur["shft_qseqs"]), dim=1).to(device)
        data_new['cc'] = torch.cat((dcur["cseqs"][:,0:1],  dcur["shft_cseqs"]), dim=1).to(device)
        data_new['cr'] = torch.cat((dcur["rseqs"][:,0:1], dcur["shft_rseqs"]), dim=1).to(device)
        data_new['ct'] = torch.cat((dcur["tseqs"][:,0:1], dcur["shft_tseqs"]), dim=1).to(device)
        data_new['q'] = dcur["qseqs"].to(device)
        data_new['c'] = dcur["cseqs"].to(device)
        data_new['r'] = dcur["rseqs"].to(device)
        data_new['t'] = dcur["tseqs"].to(device)
        data_new['qshft'] = dcur["shft_qseqs"].to(device)
        data_new['cshft'] = dcur["shft_cseqs"].to(device)
        data_new['rshft'] = dcur["shft_rseqs"].to(device)
        data_new['tshft'] = dcur["shft_tseqs"].to(device)
        data_new['m'] = dcur["masks"].to(device)
        data_new['sm'] = dcur["smasks"].to(device)
        return data_new
